...
`scripts/ps1/pipeline_runner.ps1` runs the entire museum pipeline locally, in
...
```mermaid
flowchart TB
    start(["Start"]) --> bronze["bronze_load<br/>scripts/python/incremental.py"]
...
Every stage is a plain `uv run scripts/python/<name>.py` call from the project root.
...
|---|---|---|
| `bronze_load` | `uv run scripts/python/incremental.py` | — (always runs) |
| `test_bronze` | `uv run scripts/python/run_sql_tests.py --layer bronze` | `-SkipTests` |
| `build_silver_gold` | `uv run scripts/python/dbt_runner.py --skip-tests` | `-BronzeOnly` |
| `test_silver_gold` | `uv run scripts/python/run_sql_tests.py --layer silver --layer gold` | `-SkipTests` or `-BronzeOnly` |
...
```powershell
./scripts/ps1/pipeline_runner.ps1                 # full pipeline
./scripts/ps1/pipeline_runner.ps1 -SkipTests      # load + build only, no data-quality checks
./scripts/ps1/pipeline_runner.ps1 -BronzeOnly     # bronze_load + test_bronze only
./scripts/ps1/pipeline_runner.ps1 -FullRefresh    # forces a full reload through the whole chain
```
