...
WITH Source AS (
    SELECT
        ...
        NULLIF(TRIM(sale_price::TEXT),    '')::NUMERIC        AS sale_price,
        NULLIF(TRIM(regular_price::TEXT), '')::NUMERIC        AS regular_price,
        updated_at::TIMESTAMP                           AS updated_at
    FROM {{ source('bronze', 'product_size') }}
    WHERE work_id      IS NOT NULL
      AND size_id       IS NOT NULL
      AND TRIM(work_id::TEXT)       NOT LIKE '#%'
      AND TRIM(size_id::TEXT)       NOT LIKE '#%'
      AND TRIM(sale_price::TEXT)    NOT LIKE '#%'
      AND TRIM(regular_price::TEXT) NOT LIKE '#%'
),
